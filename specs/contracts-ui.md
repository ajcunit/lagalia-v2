...
- **Redisseny 2026-08-17** (mateixa estètica que la fitxa externa del SuperBuscador, components compartits `contractSheet.tsx`): **pestanyes** Resum · Documents (n) · Execució (pròrrogues+modificacions) · Historial · Tasques. Resum: columna esquerra amb **cronograma del procés** (anunci previ → publicació → licitació → adjudicació → formalització → inici → fi calculada; l'API exposa ara les quatre `*_notice_date`) i targeta de **CPVs amb descripcions del catàleg**; targeta «Informació rellevant» agrupant general/dates/imports/adjudicatari/classificació; **criteris d'adjudicació amb barres de ponderació**; lots germans. Documents: **carpetes per fase** (Folder/FolderOpen lucide, icona per tipus de fitxer, revisió legal per document). Iconografia lucide homogènia.
