...
- **Durada màxima**: `JOBS_TIMEOUT_SECONDS` (per defecte **6 h**) mana el `job_timeout` del worker. El valor per defecte d'arq són **300 s**, que tallava a mig fer els treballs llargs de veritat — l'enriquiment massiu i les sincronitzacions completes piquen la font expedient a expedient i duren hores. Quan salta, el runner deixa el job en `cancelled`.
- **Execucions òrfenes** (B-021): tota `sync_run` porta el `job_id` del treball que la fa. Si el job mor sense tancar-la (cancel·lació, temps exhaurit, reinici del worker — `CancelledError` no passa per l'`except Exception` que crida `fail_run`), l'escombrat `jobs.sweep` (cada 15 min) la passa a `failed` amb motiu «interrompuda». Les runs velles sense vincle es tanquen per antiguitat (> 24 h en `running`). Una run amb el job encara `queued`/`running` mai no es toca.
- **Independència de la sessió**: el job viu al contenidor `worker`, no a la request. Tancar la sessió, tancar el navegador o reiniciar l'API no l'atura; la petició que l'encua respon `202` immediatament.
